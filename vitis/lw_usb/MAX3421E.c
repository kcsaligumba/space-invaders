...
#include "stdio.h"
#include "string.h"
//#include "../../../../lab6_2/lab6_2_workspace/mb_usb/src/lw_usb/project_config.h"
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	// Select the MAX3421E on the SPI bus before starting the transfer.
	Status = XSpi_SetSlaveSelect(&SpiInstance, 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI slave select failed in MAXreg_wr: %d\n", Status);
		return;
	}

	// Send the write command byte followed by the value for that register.
	WriteBuffer[0] = reg + 2;
	WriteBuffer[1] = val;
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, sizeof(WriteBuffer));
	if (Status != XST_SUCCESS) {
		xil_printf("SPI transfer failed in MAXreg_wr: %d\n", Status);
	}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Select the MAX3421E and keep chip select asserted for the whole burst write.
	Status = XSpi_SetSlaveSelect(&SpiInstance, 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI slave select failed in MAXbytes_wr: %d\n", Status);
		return data;
	}

	// The first transmitted byte is the register write command.
	WriteBuffer[0] = reg + 2;

	// Copy each outgoing payload byte after the command byte.
	for (i = 0; i < nbytes; i++) {
		WriteBuffer[i + 1] = data[i];
	}

	// Transfer the command byte plus the requested payload bytes in one SPI burst.
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, (unsigned int)nbytes + 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI transfer failed in MAXbytes_wr: %d\n", Status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	// Select the MAX3421E before sending the register read command.
	Status = XSpi_SetSlaveSelect(&SpiInstance, 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI slave select failed in MAXreg_rd: %d\n", Status);
		return 0;
	}

	// Clock out the register address, then one dummy byte to receive the reply.
	WriteBuffer[0] = reg;
	WriteBuffer[1] = 0x00;
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, sizeof(WriteBuffer));
	if (Status != XST_SUCCESS) {
		xil_printf("SPI transfer failed in MAXreg_rd: %d\n", Status);
		return 0;
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Select the MAX3421E and perform the register command plus all reads as one burst.
	Status = XSpi_SetSlaveSelect(&SpiInstance, 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI slave select failed in MAXbytes_rd: %d\n", Status);
		return data;
	}

	// Send the register address first, then dummy bytes to clock back the response bytes.
	WriteBuffer[0] = reg;
	for (i = 0; i < nbytes; i++) {
	...
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, (unsigned int)nbytes + 1);
	if (Status != XST_SUCCESS) {
		xil_printf("SPI transfer failed in MAXbytes_rd: %d\n", Status);
		return data;
	}

	// Skip the first received byte because it corresponds to the command phase.
	for (i = 0; i < nbytes; i++) {
