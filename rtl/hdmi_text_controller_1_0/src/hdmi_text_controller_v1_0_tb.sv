...
module hdmi_text_controller_tb();
    ...
    task axi_read (input logic [31:0] addr, output logic [31:0] data);
        begin
            ...
            wait(read_addr_ready); // Wait for slave ready signal
            @(posedge aclk); // Ready signal and a positive edge
            read_addr_valid <= 1'b0; // Deassert address valid signal
            
            wait(read_data_valid); // Wait for slave valid signal
            @(posedge aclk); // Valid signal and a positive edge
            data = read_data; // Handshake completed
            read_data_ready <= 1'b0; // Deassert ready signal
        end
    endtask;
