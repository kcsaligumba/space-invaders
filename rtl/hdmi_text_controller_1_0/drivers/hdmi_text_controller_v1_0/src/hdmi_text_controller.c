...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	uint8_t reg_idx = color >> 1; // which of the 8 palette registers
	uint32_t current = hdmi_ctrl->PALETTE[reg_idx]; // read current value (AXI readback)
	uint32_t new_color = ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);
	if ((color & 0x1) == 0) {
		// Even color index: update lower 12 bits, preserve upper half
		current = (current & 0xFFFF0000) | new_color;
	} else {
		// Odd color index: update upper 12 bits (shifted to bits [27:16]), preserve lower half
		current = (current & 0x0000FFFF) | (new_color << 16);
	}
	hdmi_ctrl->PALETTE[reg_idx] = current;
}
