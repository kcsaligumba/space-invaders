...
#define STUDENT1NETID "ksali"
#define STUDENT2NETID "gseva"

struct TEXT_HDMI_STRUCT {
	uint8_t             VRAM [ROWS*COLUMNS*2]; //Week 2 - extended VRAM
	
	uint8_t             PADDING [0x2000 - ROWS*COLUMNS*2]; // added gap padding to align palette to 0x2000
	uint32_t            PALETTE [8]; // added 8 palette registers (16 colors), at 0x2000-0x201F
...
static volatile struct TEXT_HDMI_STRUCT* hdmi_ctrl = (struct TEXT_HDMI_STRUCT*)XPAR_HDMI_TEXT_CONTROLLER_0_AXI_BASEADDR; 
// Added explicit cast to silence compiler warning about integer-to-pointer conversion
